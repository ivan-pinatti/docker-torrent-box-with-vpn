...
#--- works, just need to redo search.py to accomodate multiple search terms
    ...
    def testjd2(self, jd2_url=None):
        import urllib.request
        try:
            req = urllib.request.Request(
                jd2_url.rstrip('/') + '/jd/version',
                headers={'User-Agent': 'Mylar'}
            )
            with urllib.request.urlopen(req, timeout=5) as resp:
                body = resp.read().decode('utf-8', errors='replace')
                if resp.status == 200:
                    logger.info('[JDownloader2] Successfully connected to %s' % jd2_url)
                    return json.dumps({'status': True, 'message': 'Successfully connected to JDownloader2'})
                return json.dumps({'status': False, 'message': 'HTTP %d' % resp.status})
        except Exception as e:
            logger.warn('[JDownloader2] Could not connect to %s: %s' % (jd2_url, e))
            return json.dumps({'status': False, 'message': str(e)})
    testjd2.exposed = True
